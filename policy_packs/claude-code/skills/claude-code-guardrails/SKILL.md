...
Use `juard explain claude-code-tool-use` to inspect its criteria and `juard history claude-code-tool-use` to inspect recent results. Use `juard get claude-code-tool-use` for the full YAML. Add `-namespace NAME` before the command when the pack was installed outside `root`. For authoring or evaluating other policies, use the `juardrails` skill.
